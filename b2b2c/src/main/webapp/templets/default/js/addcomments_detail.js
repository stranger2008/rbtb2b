///////////////////////////////加载商品评论的基本信息//////////////////////////////////
 $(document).ready(function(){
    ...
    var divcssli = {background: '#FFF7D2',width: idwidth,height:'26px'}; //添加LI的样式     
    var divcsstr = {background: '#FFF7D2',width: idwidth,height:'26px'};  //添加tr的样式
    var divcsstd ='background:#FFA40D;height:26px;';  
    ...
    //加载评论信息
 	$.ajax({  	 
            type: "post",    	     
            url: "/portal/membercomment!getCountComments.action?infotype="+info_type+"&infoid="+infoid+"&ajaxRequestRandom="+Math.random(),       
            datatype:"json",
            async:false,
       ...
       $.ajax({  	 
            type: "post",    	     
            url: "/portal/membercomment!checkCommentInfo.action"+"&ajaxRequestRandom="+Math.random(),       
            datatype:"json",
            async:false,
            success: function(data){
               //若为2说明用户已登录
...
function addcommentss()
    {    
     ...
     var comment=$("#comment").val();//获取是否允许评论
     ...
     $.ajax({  	 
            type: "post",    	     
            url: "/portal/membercomment!checkLogin.action?ajaxRequestRandom="+Math.random(),       
            datatype:"json",
            async:false,
            success: function(data){
            	if(data=='0'){
     ...
     $.ajax({  	 
            type: "post",    	     
            url: "/portal/membercomment!checkCommentInfo.action?commentrand="+commentrand+"&ajaxRequestRandom="+Math.random(),       
            datatype:"json",
            async:false,
            success: function(data){
               if(data=="1"){
...
function gbcount()
{
 ...
 function addSupport(id)
 {
  ...
  $.ajax({  	 
            type: "post",    	     
            url: "/portal/membercomment!addSupport.action?commid="+id+"&ajaxRequestRandom="+Math.random(),       
            datatype:"json",
            async:false,
            success: function(data){
               if(data=="1")
