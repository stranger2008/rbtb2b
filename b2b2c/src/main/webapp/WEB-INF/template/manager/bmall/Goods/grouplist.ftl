<head>
	...
	<script type="text/javascript">
		  ...
		  function selInfo(field_name,obj){  
			  ...
			  var radio_val=$('input:radio[name="goods.goods_id"]:checked').val();     
			  ...
			  if(flag==true){
				  ...
				  yesFn: function () {
				     var id="";
				     var name="";
				     var vals=radio_val.split("######");
				     if(vals.length>1){
				     	id=vals[0];
				     	name=vals[1];
				     }
   			        opener.setgroup(id,name);
   			        window.close();
				    return false;
				    },
				 noText: '关闭',
...
<body>
<@s.form action="/bmall_Goods_grouplist.action"  method="post">
     ...
     <div class="ropot">
       ...
       <table cellspacing="0" class="bmall_list_table">
         <tr>
	         ...
	         <td class="fthstyle1">商品名称</td>
	         <td class="fthstyle1"  width="20%">所属分类</td>	         
	         <td class="fthstyle1"  width="10%">商品品牌</td>
	         <td class="fthstyle1"  width="10%">销售价（元）</td>
	         <td class="fthstyle1"  width="10%">免运费</td>
	         <td class="fthstyle1"  width="10%">状态</td>
         <tr>
          <tr><td colspan="9"  class="manadiv">
			<input type="button" value="选中" class="submitbut" onclick="selInfo('goods.goods_id',this);"/>
         </td></tr>   
	     <#list goodsList as goods>
<tr>
    <td><input type="radio" class="ch_x" name="goods.goods_id" value="${goods.goods_id?if_exists}######${goods.goods_name?if_exists}"/>
