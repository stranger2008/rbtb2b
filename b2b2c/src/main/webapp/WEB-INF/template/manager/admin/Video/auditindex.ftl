<html>
  ...
  <body>
<@s.form action="/admin_Video_auditList.action" method="post">
<@s.hidden name="video.is_recom" id="admin_video_state"/>
<div class="main_index f_left">
 <div class="pageLocation">
 	当前位置:功能模块 > 视频管理 > 视频审核列表
   </div>
 <div class="main_top">
   <ul class="main_top_ul">    
     <li class="shuaix"><a onclick="showSearch(this,'searchDiv');">筛选</a></li>
     <li class="sc"><a onclick="delInfo('video.video_id','/admin_Video_checkDel.action')">删除</a></li>
   </ul>
 </div>
 <div class="main_cont">
  ...
  <tr>
    ...
    <#if video.title?length lt 6>
    <a onclick="linkToInfo('/admin_Video_audit.action','video.video_id=${video.video_id?if_exists}');">${video.title?if_exists}</a><#if video.is_recom?if_exists=='1'><a onclick="linkToInfo('/admin_Video_auditList.action','is_recom_s=${video.is_recom?if_exists}');"><span class="redcolor">[推荐]</span></a></#if>
    <#else>
    <a onclick="linkToInfo('/admin_Video_audit.action','video.video_id=${video.video_id?if_exists}');">${video.title[0..5]}</a><#if video.is_recom?if_exists=='1'><a onclick="linkToInfo('/admin_Video_auditList.action','is_recom_s=${video.is_recom?if_exists}');"><span class="redcolor">[推荐]</span></a></#if>
    </#if>
    </#if></td>
    ...
    <td align="center">
             <a onclick="linkToInfo('/admin_Video_auditList.action','cat_attr_s=${video.cat_attr_id?if_exists}');">${video.cat_attr?if_exists}</a>
    </td>
    <td align="center">
    <a onclick="linkToInfo('/admin_Video_auditList.action','info_state_s=${video.info_state?if_exists}');">
    ...
    <td align="center"><a onclick="linkToInfo('/admin_Video_audit.action','video.video_id=${video.video_id?if_exists}');"><img src="/include/images/audit.png" /></a></td>
  </tr>
  </#list>
