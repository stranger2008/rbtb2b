<html>
  ...
  <body>
<@s.form action="/admin_Classified_auditList.action" method="post">
<div class="main_index f_left">
 <div class="pageLocation">
 	   当前位置:功能模块 > 分类信息管理 > 审核分类信息列表
 </div>
 <div class="main_top">
   <ul class="main_top_ul">     
     <li class="shuaix"><a onclick="showSearch(this,'searchDiv');">筛选</a></li>
     <li class="sc"><a onclick="delInfo('classified.info_id','/admin_Classified_checkDel.action')">删除</a></li>
   </ul>
 </div>
 <div class="main_cont">
  ...
  <tr>
    ...
    <a onclick="linkToInfo('/admin_Classified_audit.action','classified.info_id=${classified.info_id?if_exists}');">${classified.title?if_exists}</a><#if classified.is_recom?if_exists=='1'><a onclick="linkToInfo('/admin_Classified_list.action','is_recom_s=${classified.is_recom?if_exists}');"><span class="redcolor">[推荐]</span></a></#if>
    <#else>
    <a onclick="linkToInfo('/admin_Classified_audit.action','classified.info_id=${classified.info_id?if_exists}');">${classified.title[0..13]}</a><#if classified.is_recom?if_exists=='1'><a onclick="linkToInfo('/admin_Classified_list.action','is_recom_s=${classified.is_recom?if_exists}');"><span class="redcolor">[推荐]</span></a></#if>
    </#if>
    </#if></td>
    
    <td align="center">
    <#if classified.cust_name?if_exists!=''>
    <#if classified.cust_name?length lt 6>
    ${classified.cust_name?if_exists}
    <#else>
    ${classified.cust_name[0..5]}...
    </#if>
    </#if></td>
    <td align="center">
       <a onclick="linkToInfo('/admin_Classified_auditList.action','cat_attr_s=${classified.cat_attr_id?if_exists}');">${classified.cat_attr?if_exists}</a>
    </td>
    ...
    <td align="center"> <a onclick="linkToInfo('/admin_Classified_audit.action','classified.info_id=${classified.info_id?if_exists}');"><img src="/include/images/audit.png" /></a></td>
  </tr>
  </#list>
