...
<div class="tj_main f_left">
 <div class="pageLocation">
 	当前位置:会员管理 > 会员管理 > 会员级别管理 > 修改会员级别
 </div>
   <div class="tj_main_cont">
   
   	<@s.form action="/admin_Memberlevel_update.action" method="post" validate="true">
        ...
        <table class="wwtable" cellspacing="1" cellpadding="8">
           ...
           <tr>
		    ...
		    <#list menuList as menu>	
              d.add(${menu.menu_id?if_exists},${menu.up_menu_id?if_exists},'<input type="checkbox" id="${menu.up_menu_id?if_exists}${menu.menu_id?if_exists}" name="nameright" value="${menu.menu_id?if_exists}"/> ${menu.menu_name?if_exists}&nbsp;${stack.findValue("@com.rbt.function.MemberRightFuc@getMenuRightListByMenuid('${menu.menu_id?if_exists}')")}');
		    </#list>
		    		
		    document.write(d);
		
	        </script>
                <@s.fielderror><@s.param>memberlevel.menu_right</@s.param></@s.fielderror></div>
             </td>
           </tr>
            ...
            <tr>
             ...
             <ul style="width:800px;">
	             	<#list controlList as c>
	             		<li style="float:left;margin-right:10px;margin-bottom:10px;white-space: nowrap;width:240px;">
	     ...
	     <div class="buttom">
	       ...
	       <@s.hidden name="memberlevel.level_id" />
	       <@s.hidden name="memberlevel.syslevel" />
	       <@s.submit value="保存"/>
	       <@s.hidden id="mr_list" value="${memberlevel.menu_right?if_exists}"/>
	       <@s.hidden id="op_list" value="${memberlevel.oper_right?if_exists}"/>
	       <@s.reset value="重置"/>
