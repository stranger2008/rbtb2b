...
<body>
 <div class="cont_main">
 <@s.form action="/member_Showinfo_update.action" method="post" validate="true">
      <table width="100%" class="cont_title">
 	<tr>
    	<td>当前位置:信息管理>展会管理>修改展会</td>
 	</tr>
	</table>
  <table width="100%" border="0" bgcolor="#DDDDDD" cellspacing="1" class="main_cont">
   <tr <#if showinfo.info_state?if_exists='2'><#else>style='display:none;'</#if> >
    <td width="17%" valign="middle" class="left_td" style="font-weight:bold;">未通过理由：</td>
    <td width="83%" colspan="3">
    ${showinfo.no_reason?if_exists}
    </td>
  </tr>
  <tr>
    ...
    <td colspan="3">
	    <#if sysmodule.is_catattr=="0">             
	                 ...
	                 <@s.hidden name="showinfo.cat_attr" />
              		<a href="/member_Showinfo_cate.action?cat_attr=0&showinfo.exh_id=${showinfo.exh_id?if_exists}">[重新选择]</a>
	            <#else>
		            <table>
	             		<tr>
	             			<td class="tdbottom">
  ...
  <tr>
    ...
    <td colspan="3">
     <#if showinfo.start_date?if_exists?length lt 10>
      ...
      <@s.textfield id="txtstartDate" name="showinfo.start_date"  type="text" cssStyle="width:100px;" cssClass="Wdate" 
		               readonly="true" onclick="WdatePicker({maxDate:'#F{$dp.$D(\\'txtendDate\\',{d:1})}',readOnly:true})"  /> 
       <#else>
         <@s.textfield id="txtstartDate" value="${showinfo.start_date?if_exists[0..9]}" name="showinfo.start_date"  type="text" cssStyle="width:100px;" cssClass="Wdate" 
		              readonly="true" onclick="WdatePicker({maxDate:'#F{$dp.$D(\\'txtendDate\\',{d:1})}',readOnly:true})"  />         
      </#if>
  ...
  <tr>
    ...
    <td colspan="3">
     ...
     <@s.textfield id="txtendDate" name="showinfo.end_date" cssStyle="width:100px;"  type="text" cssClass="Wdate" 
		               readonly="true" onclick="WdatePicker({minDate:'#F{$dp.$D(\\'txtstartDate\\',{d:1})}',readOnly:true})"  />
       <#else>
        <@s.textfield id="txtendDate" value="${showinfo.end_date?if_exists[0..9]}" name="showinfo.end_date" cssStyle="width:100px;"  type="text" cssClass="Wdate" 
		               readonly="true" onclick="WdatePicker({minDate:'#F{$dp.$D(\\'txtstartDate\\',{d:1})}',readOnly:true})"  />      
      </#if>
  ...
  <tr>
    ...
    <td colspan="3">
    <@s.textarea name="showinfo.exh_desc" id="content" cssClass="txtinput" cssStyle="width:300px;height:200px;"value="${showinfo.exh_desc}"  />
  ...
  <tr>
    ...
    <@s.hidden name="token_value" value="${get_token_value?if_exists}"/>
            ...
            <@s.hidden name="showinfo.is_recom"/>
            <@s.hidden name="showinfo.clicknum"/>
            <@s.hidden name="showinfo.fare"/>
