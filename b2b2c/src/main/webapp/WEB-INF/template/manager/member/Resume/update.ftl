...
<body>
 ...
 <@s.form action="/member_Resume_update.action" method="post" validate="true">
 	...
 	<tr>
    	<td>当前位置:信息管理>简历管理>修改简历</td>
 	</tr>
	</table>
  <table width="100%" border="0" bgcolor="#DDDDDD" cellspacing="1" class="main_cont">
   <tr <#if resume.info_state?if_exists='2'><#else>style='display:none;'</#if> >
    <td width="17%" valign="middle" class="left_td" style="font-weight:bold;">未通过理由：</td>
    <td width="83%">
    ${resume.no_reason?if_exists}
    </td>
  </tr>
    <tr>
    ...
    <td>
	     <#if sysmodule.is_catattr=="0">             
	                 ...
	                 <@s.hidden name="resume.cat_attr" />
              		<a href="/member_Resume_cate.action?cat_attr=0&resume.resume_id=${resume.resume_id?if_exists}">[重新选择]</a>
	            <#else>
		            <table>
	             		<tr>
	             			<td class="tdbottom">
 ...
 <@s.select  name="resume.job_type" cssStyle="width:100px;"  list=r"#{'不限':'不限','全职':'全职','兼职':'兼职','实习':'实习'}"  />
   ...
   <@s.textfield name="resume.language" cssClass="txtinput"  maxLength="100"  cssStyle="width:300px;"/>
    ...
    <td>
   <@s.textfield name="resume.cellphone" cssClass="txtinput" maxLength="20"  cssStyle="width:300px;"/>
   ...
   <tr>
    ...
    <td>
    <@s.radio name="resume.is_trust" cssClass="txtinput" list=r"#{'0':'否','1':'是'}" /> 
    </td>
  <tr>
  <tr>
    <td colspan="4" class="subbut">
     <@s.hidden name="token_value" value="${get_token_value?if_exists}"/>
            <@s.hidden name="resume.is_recom" />
            <@s.hidden name="resume.clicknum" />
            <@s.hidden name="resume.fare" />
