...
<body>
 ...
 <@s.form   method="post" validate="true">
  ...
  <tr>
    <td valign="middle" class="left_td">应聘会员:</td>
    <td>
   <@s.label name="resumeinbox.user_name"/>
   </td>
  </tr>
  <tr>
    ...
    <@s.hidden name="token_value" value="${get_token_value?if_exists}"/>
	       ...
	       <#if resumeinbox.isinvite?if_exists='0'>
	       <input type="button" name="invite" class="sub" value="面试邀请" onclick="linkToInfo('/member_Resumeinbox_invite.action','resumeinbox.inbox_id=${resumeinbox.inbox_id?if_exists}');"/>
	       </#if>
	       <input type="button" name="returnList" class="sub" value="返回列表" onclick="linkToInfo('/member_Resumeinbox_list.action','');"/>
    </td>
  </tr>
</table>
 </@s.form>
