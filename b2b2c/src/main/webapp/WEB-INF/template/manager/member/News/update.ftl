...
<body>
 <div class="cont_main">
 <@s.form action="/member_News_update.action" method="post" validate="true">
     <table width="100%" class="cont_title">
 	<tr>
    	<td>当前位置:信息管理>资讯管理>修改资讯</td>
 	</tr>
	</table>
  <table width="100%" border="0" bgcolor="#DDDDDD" cellspacing="1" class="main_cont">
   <tr <#if news.state_code?if_exists='2'><#else>style='display:none;'</#if> >
    <td width="17%" valign="middle" class="left_td" style="font-weight:bold;">未通过理由：</td>
    <td width="83%" colspan="3" >
   ${news.no_reason?if_exists}
    </td>
  </tr>
  <tr>
    <td width="17%" valign="middle" class="left_td">资讯标题<span class="mustfield">*</span></td>
    <td width="83%" colspan="3">
    <@s.textfield name="news.title" cssClass="txtinput" size="70" maxLength="600"/>
    ...
    <td colspan="3">
	   <#if sysmodule.is_catattr=="0">             
	                 ...
	                 <@s.hidden name="news.cat_attr" />
              		<a href="/member_News_cate.action?cat_attr=0&news.news_id=${news.news_id?if_exists}">[重新选择]</a>
	            <#else>
		            <table>
	             		<tr>
	             			<td class="tdbottom">
  ...
  <tr>
    <td colspan="4" class="subbut">
		   ...
		   <@s.hidden name="news.keyword"/>
		   <@s.hidden name="news.news_attr"/>
		   <@s.hidden name="news.source"/>
		   <@s.hidden name="news.comment"/>
		   <@s.hidden name="news.vote_id"/>
		   <@s.hidden name="news.out_link"/>
		   <@s.hidden name="news.sort_data_number"/>
		   <@s.hidden name="news.out_link"/>
	      ...
	      <!--所属分类插件隐藏字段开始区域-->
		   ...
		   <!--所属地区插件隐藏字段结束区域-->
	        <input type="button" name="returnList" value="返回列表" class="sub" onclick="linkToInfo('/member_News_list.action','');"/>
    </td>
  </tr>
</table>
