...
<title>修改产品</title>
   <script src="/include/js/jquery-1.4.4.min.js" type="text/javascript"></script>
    ...
    <script src="/include/js/member/product.js" type="text/javascript"></script>
...
<body>
 <div class="cont_main">
 <@s.form action="/member_Product_update.action" method="post" validate="true">
       <table width="100%" class="cont_title">
 	<tr>
    	<td>当前位置:信息管理>产品管理>修改产品</td>
 	</tr>
	</table>
  <table width="100%" border="0" bgcolor="#DDDDDD" cellspacing="1" class="main_cont">
   <tr <#if product.info_state?if_exists='2'><#else>style='display:none;'</#if> >
    <td width="17%" valign="middle" class="left_td" style="font-weight:bold;">未通过理由：</td>
    <td width="83%">
    ${product.no_reason?if_exists}
    </td>
  </tr>
  <tr>
    ...
    <td>
	    <#if sysmodule.is_catattr=="0">             
	                 ...
	                 <@s.hidden name="product.cat_attr" />
              		<a href="/member_Product_cate.action?cat_attr=0&product.product_id=${product.product_id?if_exists}">[重新选择]</a>
	            <#else>
		            <table>
	             		<tr>
	             			<td class="tdbottom">
  ...
  <@s.textfield name="product.brand" cssClass="txtinput" cssStyle="width:200px;" maxLength="50"/>
    ...
    <td >
     <@s.textarea name="product.content" id="content" cssClass="txtinput" cssStyle="width:300px;height:200px;" value="${product.content?if_exists}"  />
  ...
  <tr>
    ...
    <@s.hidden name="token_value" value="${get_token_value?if_exists}"/>
           <@s.hidden name="area_attr"/>
           <@s.hidden name="product.clicknum" />
           <@s.hidden name="product.fare" />
           <@s.hidden name="product.cust_id" />
           <@s.hidden name="product.is_recom"/>
