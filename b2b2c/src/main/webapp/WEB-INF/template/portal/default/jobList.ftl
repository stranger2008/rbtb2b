...
<body>
<@s.form action="/portal/job!list.action" method="post" name="searchForm">
...
<div  id="renposition" class="left_main f_left">
    ...
    <div class="dldetail_title" id="dldetail_title"><ul>
      <li ><a href="/portal/resume!list.action">人才搜索</a></li>
      <li class="dldetail_title_h"><a href="/portal/job!list.action">职位搜索</a></li></ul>
    </div>
<div class="dldetail_main">
      <div class="f_left dldetail_n"><p class="f_left">工作性质</p>
      	  <@s.select name="worktype" list=r"#{'':'--请选择--','不限':'不限','全职':'全职','兼职':'兼职','实习':'实习'}" id="id_worktpye"/>
      </div>      
      <div class="f_left dldetail_n"><p class="f_left">性别要求</p>
          <@s.select name="gender" list=r"#{'':'--请选择--','不限':'不限','男':'男士','女':'女士'}" id="id_gender"/>
      </div>
      <div class="f_left dldetail_n"><p class="f_left">婚姻状况</p>
	      <@s.select name="marriage" list=r"#{'':'--请选择--','不限':'不限','未婚':'未婚','已婚':'已婚'}" id="id_marriage"/>
      </div>
      <div class="f_left dldetail_n"><p class="f_left">学历要求</p>
      <@s.select name="education" list=r"#{'':'--请选择--','不限':'不限','初中':'初中','高中':'高中','大专':'大专','本科':'本科','硕士':'硕士','博士':'博士'}" id="id_education"/>
      </div>
...
<#if positionList?if_exists &&(positionList?size > 0 )>    
    <div class="list_title"><h2 class="title_text1">职位名称<span class="list_title_edu">公司名称</span>
    <span class="list_title_work">工作地点</span><span style='padding-right:12px;'>发布日期</span></h2></div>
    <div class="list_main">
    <ul>
     <#list positionList as post>
        ...
        <li <#if post_index%2!=0> class="list_main_n2"</#if>>
	        ...
	        <span class="list_main_n1_text" >        
	        	<a href="${stack.findValue("@com.rbt.function.ChannelFuc@getArticleUrl('job','${post.job_id?
				if_exists}','${rbttime?if_exists}')")}" target="_blank">
				
				<#if post.title?length lt 12>
			      ${post.title?if_exists}
			    <#else>
			      ${post.title[0..11]}...
			    </#if>		
	        ...
	        <span class="list_main_major">${post.cust_name?if_exists}</span>
	        <span class="list_main_work"">${post.area_name?if_exists}</span>
   ...
   <div class="right_main f_right">
      <div class="area_title"><h2 class="title_text">热点招聘</h2></div>
       <ul class="ph_main">
        <#list hotsearchList as hot>
          <#assign rbttime='${(hot.in_date)?if_exists}'/>  
         <li><a href="${stack.findValue("@com.rbt.function.ChannelFuc@getArticleUrl('job','${hot.job_id?
if_exists}','${rbttime?if_exists}')")}" target="_blank"><span class="area_main_major">
         <#if hot.title?length lt 18>${hot.title?if_exists}<#else> ${hot.title[0..17]}...</#if>
         </span></a></li>        
        </#list>
   </ul>
   <div class="ad">
   <script src="/include/advshow.jsp?pos_id=114"></script>
   </div>
    <div class="area_title"><h2 class="title_text">推荐招聘</h2></div>
      <ul class="area_main">
        <#list recomsearchList as recom>
           <#assign rbttime='${(recom.in_date)?if_exists}'/>  
           <li><a href="${stack.findValue("@com.rbt.function.ChannelFuc@getArticleUrl('job','${recom.job_id?
if_exists}','${rbttime?if_exists}')")}" target="_blank"><span class="area_main_major">
           <#if recom.title?length lt 18>${recom.title?if_exists}<#else> ${recom.title[0..17]}...</#if></span></a></li>       
        </#list>
   </ul>
   <div class="ad">
   <script src="/include/advshow.jsp?pos_id=115"></script>
   </div>
   <div>
   <script src="/include/advshow.jsp?pos_id=116"></script>
   </div>
   <ul class="text_ad">
  </div>
</div>
...
<style type="text/css"/>
.dldetail_main select{width:120px;}
</style>
