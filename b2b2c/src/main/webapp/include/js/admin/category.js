...
{    
 ...
 //删除菜单名
 function del(id) { 
   hiConfirm('您要删除这些选项吗，有可能会对子分类造成影响.确定要删？', '友情提示',function(r){ 
         if(r){ 
           $.ajax({  	 
              type: "post",    	     
...
function creatediv(level,num,title,id){
   ...
   "<div class=\"catelist\"><li class='cateli' style='text-align:center;width:260px' >您还未添加子分类</li></div><h3><img class='addclass' title='添加当前列表的子分类' onclick='addcate("+cat_level+")' src='/include/images/add.png'/>"+
   "</h3></div>")
   $("#menuvalue"+(level+1)).val(id);
