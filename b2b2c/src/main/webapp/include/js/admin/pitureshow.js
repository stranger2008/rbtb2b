...
//初始化页面加载图片
//imgurl：初始化图片地址
//addimgid：处理后绑定图片的DIV的ID
//imgwidths:显示图片的宽度
//imgheights:显示图片的高度
function firstaddimges(imgurlid,addimgid,imgwidths,imgheights)
{
  var imgurl=$("#"+imgurlid).val();
  var retimgurl="";
  var adddiv1="";
  var basedivstart="<span style='border:2px;margin-left:5px;margin-right:5px;'><img width='"+imgwidths+"' height='"+imgheights+"' border='0' src='";
  var basedivend="'></span>";
  var strimgs="";
  if(imgurl!=null&&imgurl!="")
  {
    if(imgurl.indexOf(",")>-1)
    {
      strimgs=imgurl.split(',');
      for(i=0;i<strimgs.length;i++)
      {
       retimgurl+=basedivstart+strimgs[i].toString()+basedivend;
       
      }
    }
    else
    {
     retimgurl=basedivstart+imgurl+basedivend;
    }
  }
  $("#"+addimgid).html(retimgurl);
  pictureshowbig(addimgid);
}
//获取所有图片路径
function pictureshowbig(imgdivid)
{
   
	 var imgList2=document.getElementById(imgdivid).getElementsByTagName("img");
