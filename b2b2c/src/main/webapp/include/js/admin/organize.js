...
function showorga(org_id,level){	 
     //加载颜色分别
      ...
      $.ajax({  	 
          type: "post",    	     
          url: "/admin_Organize_getList.action?up_org_id="+org_id+"&ajaxRequestRandom="+Math.random(),                 
          datatype:"json",
   ...
   //删除组织结构
   function deleteorga(area_id,level){
       hiConfirm('您要删除这些选项吗，有可能会对子选项造成影响.确定要删？', '友情提示',function(r){ 
       if(r){ 
         $.ajax({  	 
            type: "post",    	     
            url: "admin_Organize_org_delete.action?id="+area_id+"&ajaxRequestRandom="+Math.random(),    
            datatype:"json",
            success: function(data){   
...
function onlyshoworg(id,level){
       $.ajax({  	 
           type: "post",    	     
           url: "admin_Organize_viewlist.action?cid="+id+"&ajaxRequestRandom="+Math.random(),       
           datatype:"json",
           async:true,
           success: function(data){ 
            if(data.length>0)
...
//根据上级分类ID找出本级的分类添加到select中
 function getThisOrg(up_org_id){
 	$.ajax({  	 
            type: "post",    	        
            url: "admin_Organize_viewlist.action?cid="+up_org_id+"&ajaxRequestRandom="+Math.random(),       
            datatype:"json",
            async:false,
            success: function(data){      
   	           $("#orgselect").append(data);
