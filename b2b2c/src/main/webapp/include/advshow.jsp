...
<% 
	...
	if(!pos_id.equals("")){
		...
		advList = AdvinfoFuc.getDisplayAdv(pos_id,cat_id,"cate");
		 ...
		 advList = AdvinfoFuc.getDisplayAdv(pos_id,"","cate");
		}
		if(advList!=null && advList.size()>0){
			...
			if(pos_type.equals("a")){
				...
				out.println("document.write('<a href=\"/portal/advpos!updateaddnum.action?link_url="+link_url+"&pos_id=" + pos_id + "\" target=\"_blank\" "+class_style+" title=\""+content+"\">"+title+"</a>');");
			}
			else if(pos_type.equals("c")){
				//图片广告
				img_path = img_path.replace("'","");
				out.println("document.write('<a href=\"/portal/advpos!updateaddnum.action?link_url="+link_url+"&pos_id=" + pos_id + "\" target=\"_blank\"><img src=\""+img_path+"\" "+class_style+" width=\""+p_width+"\" height=\""+p_height+"\"/></a>');");
			}
