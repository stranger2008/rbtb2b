...
public class SpecialField {
	...
	public String getArticleVlaue(String name, Map map) {
		...
		if (name.equals("get_first_cat")) {
			...
			if (map.get("cat_attr") != null) {
				cateName = map.get("cat_attr").toString();
				cateName = CategoryFuc.getFiresCateName(cateName);
			}
		...
		if (name.equals("get_count_info")) {
			...
			value = getInfoCommentCount(mymap);
		}
		// 通过客户cust_id获取公司名称
	...
	public String getListVlaue(String name, Map map) {
		...
		if (name.equals("client_status")) {
			...
			if (map.get("client_status") != null) {
				clientName = map.get("client_status").toString();
				clientName = MemberFuc.getStatusName(clientName);
			}
			return clientName;
		}
	...
	// 获取商品评论数量
	public String getInfoCommentCount(HashMap map) {
		Integer countString = 0;
		IMembercommentService comment_Service = (IMembercommentService) CreateSpringContext.getContext().getBean("membercommentService");
		countString = comment_Service.getCount(map);
