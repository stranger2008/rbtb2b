...
import com.rbt.service.impl.CategoryService;
 ...
 */
@Controller
public class CategoryattrAction extends BaseAction implements Preparable{
	 ...
	 * 定义搜索框的搜索类型
	 */
	public String attr_name_s;
	public String attr_type_s;
	public String cat_attr_s;
	public String area_attr_s;
	public String is_must_s;	
	public String code_value;
	public String codehid;
	public String modtype_s;
	public String intr_type;
	public String level;
	
	//模块ID
	public String modtype_name_id;
	//模块名称
	public String modtype_name;
	
	public static final String UP_CAT_ID="1111111111";
	...
	public String add() throws Exception {
		...
		if((url_up_id!=null && !"".equals(url_up_id)) && (level!=null && !"".equals(level))){
			//等级为一级  level页面传值
			if("1".equals(level)){
				hidden_cat_value=UP_CAT_ID+","+url_up_id;
				cat_value = hidden_cat_value.substring(11, hidden_cat_value.length());
			}
			//等级超过2级
			else{
				now_up_id = url_up_id;
				for(int i=1;i<(Integer.parseInt(level));i++){
					category = categoryService.get(now_up_id);
					if(category==null){
				...
				hidden_cat_value=UP_CAT_ID+","+up_level+url_up_id;
				cat_value = hidden_cat_value.substring(11, hidden_cat_value.length());
			}		
		}		
	...
	public String view() throws Exception {
		...
		backCategory(categoryattr.getCat_attr());	
		//根据属性id获取属性值列表
		Map map=new HashMap();
	 ...
	 * @author  创建人  林俊钦
	 * @date  创建日期  Jul 20, 2011 2:36:32 PM
	 */	
	...
	public String list() throws Exception {
		...
		categoryattrList = categoryFuc.replaceList(categoryattrList, "");
		//系统模块列表
		Map modMap=new HashMap();
		moduleList=this.sysmoudleService.getList(modMap);
