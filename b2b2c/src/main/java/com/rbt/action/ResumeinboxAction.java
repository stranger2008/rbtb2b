 ...
 */
@Controller
public class ResumeinboxAction extends BaseAction implements Preparable {
	...
	public String invite() throws Exception{
		if(resumeinbox.getInbox_id()!=null && !"".equals(resumeinbox.getInbox_id())){
			String inbox_id=resumeinbox.getInbox_id();
			Resumeinbox resumes = new Resumeinbox();
			resumes=resumeinboxService.get(inbox_id);
			resumes.setIsinvite("1");
			this.resumeinboxService.update(resumes);
			resumeinbox=this.resumeinboxService.get(inbox_id);
			this.addActionMessage("面试邀请成功");
			return view();
		}
		else{
			return list();
