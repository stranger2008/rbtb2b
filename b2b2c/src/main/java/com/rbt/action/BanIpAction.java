 ...
 */
@Controller
public class BanIpAction extends BaseAction implements Preparable{
	...
	public String delete() throws Exception {
		String ipid = this.banip.getIp_id();
		ipid = ipid.replace(" ", "");
		this.ban_ipService.delete(ipid);
	...
	public String updateAllIP() throws Exception {
		String banipid = this.admin_ip_id;
		String banip = this.admin_ip;
		...
		if (banipidStr.length > 0) {
			...
			for (int i = 0; i < banipidStr.length; i++) {
				...
				banipMap.put("user_id", this.session_user_id);
