...
public class SessionIterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation actionInvocation) throws Exception {
		...
		if(loc.equals("") && reqUrl.endsWith(".action")){
			loc = reqUrl;
		}
		...
		//运营商不能进入会员后台
		//运营商不能进入商城会员后台
		if(cust_type.equals(Constants.ADMIN_TYPE) && reqUrl.startsWith("/"+Constants.MEMBER_TYPE)){
		...
		if (user_id == null || user_id.equals("")) {
			...
			if(reqUrl.startsWith("/"+Constants.ADMIN_TYPE)){
				return Constants.ADMIN_LOGIN;
			}else if(reqUrl.startsWith("/"+Constants.MEMBER_TYPE)){
			//该操作员为会员
				if(!loc.equals("")){
			...
			//该操作员为商城会员
				if(!loc.equals("")){
					res.sendRedirect(signinPage+"?loc="+loc);
				}
				return Constants.MEMBER_SIGNIN;
			}
		} else {
